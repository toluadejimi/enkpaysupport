...
<div class="view-post-reply">
    <div class="repley-btu-check">
        <div class="aiReaplayView">
            ...
            <nav>
                ...
                <input type="hidden" id="generateAiReplayRoute" value="{{route('admin.ai-replay-generate')}}">
                <input type="hidden" id="aiReplayDeleteRoute" value="{{route('admin.ai-replay-delete')}}">
    ...
    <div class="tab-content" id="nav-tabContent">
        <div class="tab-pane fade show active" id="nav-home2" role="tabpanel" aria-labelledby="nav-home-tab2"
             tabindex="0">
            <form action="{{route('admin.conversations.conversationStore',encrypt($ticketData->id))}}" method="post"
                    ...
                    <div class="d-flex gap-2 ticket-status-change">
                        <div class="quickVieew">
                            <input type="hidden" id="ticketStatusChangeRoute"
                                   value="{{ route('admin.tickets.ticketStatusUpdate') }}">
                            ...
                            <div class=" checkrowed"
                                ...
                                <input type="radio" class="status{{STATUS_INPROGRESS}}"
                                       @if($ticketData->status==STATUS_INPROGRESS) checked="checked"
                                       @endif id="processing"
                                       name="ticket_status">
                    ...
                    <div class="view-assign-tab " id="noteBox">
                        ...
                        <div class="all-user-search">
                            @foreach($instantMessageData as $istMsg)
                                <div class="view-user-list clock-instant d-flex justify-content-between align-items-center gap-2">
                                <div class="align-items-start d-flex gap-2 w-100">
                                    ...
                                    <div class="d-flex gap-2">
                                        <button type="button" class="border-0 p-0 bg-transparent font-11 text-secondary" onclick="editInstantMessage('{{$istMsg->id}}', '{{$istMsg->title}}', '{{$istMsg->message}}')"><i class="fa-solid fa-pen"></i></button>
                                        <button type="button" class="border-0 p-0 bg-transparent font-11 text-danger" onclick="deleteItem('{{route('admin.conversations.instant.message.delete', $istMsg->id)}}','',true)"><i class="fa-solid fa-trash"></i></button>
                                    </div>
                ...
                <div class="ticket-upload-box">
                    <div class="ticket-upload-btn-box">
                        <p class="create-ticket-image-type">{{__("Upload file")}} {{_('(JPG, JPEG, PNG, ZIP, MP4, GIF, DOC)')}}</p>
                        <div class="mb-10 align-items-center gap-2 d-flex justify-content-between displayNot">
                            ...
                            <div class="submitReaply mb-0">
                                <button type="submit" @if($ticketData->status==STATUS_CLOSED) disabled
                                        @endif class="ticket-btu-com">{{__('Reply Now')}}</button>
                            </div>
                        </div>
                    </div>
...
{{--<input type="hidden" id="checkCollisionDetector" value="{{route('admin.check-collision-detector')}}">--}}
