                ...
                <div class="auth-user-info ">
                    ...
                    <div class="d-flex align-items-start gap-3">
                        <div class="post-date">
                            ...
                            <p class="text-center">{{\Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $conversation->created_at )->diffForHumans()}}</p>
                        </div>
                        @if($conversation->user->role != USER_ROLE_CUSTOMER)
                            <div class="dropdown text-end">
                                <button class="border-0 p-0 bg-transparent dropdown-toggle conversion-list-dropdown-toggle"
                                        type="button" data-bs-toggle="dropdown" aria-expanded="false">
                                    <i class="fa-solid fa-ellipsis"></i>
                                </button>
                                <ul class="dropdown-menu dropdown-menu-end">
                                    <li>
                                        <button class="dropdown-item edit-modal-action" data-content="{{$conversation->body}}"
                                                data-id="{{$conversation->id}}">{{__("Edit")}}</button>
                                    </li>
                                    <li>
                                        <button class="dropdown-item delete-action"
                                                data-id="{{$conversation->id}}">{{__("Delete")}}</button>
                                    </li>
                                </ul>
                                <input type="hidden" value="{{route('agent.conversations.conversation-delete')}}"
                                       id="conversation-delete-Route">
                            </div>
                        @endif
                    </div>
                </div>
                <p>{!!nl2br($conversation->body)!!}</p>
