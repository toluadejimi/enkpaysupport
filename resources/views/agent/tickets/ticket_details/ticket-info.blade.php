    ...
    @if(count($ticketDynamicFieldData)>0)
        @foreach($ticketDynamicFieldData as $field)
            <div class="ticket-info">
                ...
                <div>
                    <span class="ticket-details-dynamic-filed-edit-action"  data-id="{{$field->id}}" data-value="{{$field->field_value??""}}" data-filed_type="{{$field->type}}" data-level="{{$field->level}}" data-required="{{$field->required}}">
        ...
        @endforeach
    @endif
    @if($envatoConfigData?->enable_purchase_code == 1)
        <div class="ticket-info">
            <div>
                <h5>{{__('Licence')}}:</h5>
            ...
            <div>
                                <span class="license-edit-action"  data-id="{{$ticketData->id}}" data-value="{{$ticketData->envato_licence??""}}">
                          <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 25 25" fill="none">
                            <rect width="25" height="25" rx="5" fill="#6659FF"/>
                            <g clip-path="url(#clip0_846_1766)">
                              <path
