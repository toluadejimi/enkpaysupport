                    ...
                    <div class="progres-stype">
                        <form action="{{ route('process-update') }}" method="POST">
                            ...
                            @if(config('app.build_version') == getOption('build_version'))
                                <p class="me-2 my-2"><span class="text-danger">*</span>{{__("Your application is upto date")}}</p>
                            @else
                                <p class="me-2 mb-2"><span class="text-danger">*</span> {{__("New version")}} {{ config('app.current_version') }} </p>
                                <p class="me-2 mb-2"><span class="text-danger">*</span> {{__("Current version")}} {{ getOption('current_version', '1.0') }} </p>
                                <p class="me-2 mb-2"><span class="text-danger">*</span> {{__("Download your database and present script to avoid any errors. (Safety first)")}} </p>
                                <p class="me-2 mb-2"><span class="text-danger">*</span> {{__("Please click Update now button, may its need sometime")}}</p>
                                <div class="mt-3">
                                    <div class="single-section">
                                        <h4 class="section-title mb-2">{{__("Please enter your Item purchase code and customer email")}}</h4>
                                        <div class="row">
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    <label for="email">{{__("Customer E-mail")}}</label>
                                                    <input type="email" class="form-control" id="email" name="email" value="{{ old('email') }}" placeholder="{{__("Customer E-mail")}}" />
                                                </div>
                                                @if($errors->has('email'))
                                    ...
                                    <button class="primary-btn next" id="submitNext" type="submit">{{__("Update Now")}}</button>
                                </div>
                            @endif
                        </form>
                    </div>
