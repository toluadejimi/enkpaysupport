            ...
            @if (auth()->user()->announcement_seen != 1 && announcement(auth()->user()->tenant_id) != null)
                <div class="d-none d-lg-flex resetMinit">
                    ...
                    @else
                        ...
                        <button class="bg-transparent border-0 fw-medium small-bnt text-primary"
        ...
        <div class="d-flex flex-shrink-0">
            @if (auth()->user()->announcement_seen != 1)
                @if((announcement(auth()->user()->tenant_id)) != null)
                    <div class="dropdown d-inline-block d-lg-none ms-2 flex-shrink-0">
                        ...
                        <div class="dropdown-menu dropdown-menu-lg dropdown-menu-end p-0"
                            ...
                            <div>
                                <div class="d-lg-flex resetMinit ms-0">
                                    ...
                                    @if(strlen(strip_tags(announcement(auth()->user()->tenant_id))) <= 70)
                                        <p>{{ strip_tags(announcement(auth()->user()->tenant_id)) }}</p>
                                        <button type="button" class=" cross button close dismiss-buttton"
                                                id="announcement-seen-unseen" data-bs-dismiss="modal"
                                                aria-label="Close">
                                            <span class="hover-bg-color text-white fw-bold btn btn-primary"
                                                  aria-hidden="true">{{__('Close')}}</span>
                                        </button>
            ...
            <div>
                ...
                <button type="button" class="header-item noti-icon btn-sm page-header-mood theme-dark mx-1"
                        data-theme="dark" id="page-header-notifications-dropdown" data-bs-toggle="dropdown"
                        aria-expanded="false">
                ...
                <div class="dropdown-menu dropdown-menu-lg dropdown-menu-end  notificationBox"
                    ...
                    <div class="">
                        <div class="notificationsHeading">
                            <div>
                                <h5 class="m-0">
                                    {{ __('You have ' . count(userNotification('unseen')) . ' new notification') }}</h5>
                            </div>
                            @if (count(userNotification('unseen')) > 0)
                                <div>
                                    <a href="{{ route('customer.notification-mark-as-read') }}"
                        ...
                        <div class="">
                            <div class="d-grid">
                                <a class="ticket-btu-com w-100 text-center"
                                   href="{{ route('customer.all-notification') }}">
            ...
            <div class="dropdown lang d-inline-block user-dropdown ms-2 flex-shrink-0 languageDropdown">
                <button type="button" class="header-item btn-sm langBtu" id="page-header-user-dropdown"
                        data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    <img class="rounded-circle avatar-xs fit-image header-profile-user"
                         src="{{ asset(selectedLanguage()?->flag) }}" alt="Header Avatar">
                </button>
                <div class="dropdown-menu dropdown-menu-end" aria-labelledby="page-header-user-dropdown">
                    @foreach (appLanguages() as $app_lang)
                        <a href="{{ url('/local/' . $app_lang->iso_code) }}" class="notification-item"
                           title="{{ $app_lang->language }}">
                            <div class="d-flex align-items-center  gap-3">
                                <div>
                                    <img src="{{ asset($app_lang->flag) }}" class="rounded-circle avatar-xs"
                                         alt="user-pic">
                                </div>
                                <div class="flex-1">
            ...
            <div class="dropdown d-inline-block user-profile flex-shrink-0">
                <button type="button" class="header-item btn-sm sf-header-menu" id="page-header-user-dropdown"
                        data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    <img class="rounded-circle avatar-xs fit-image header-profile-user"
                         src="{{ getFileUrl(auth()->user()->image) }}" alt="Header Avatar">
...
<div class="modal fade" id="customer" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header dialog-title p-4">
