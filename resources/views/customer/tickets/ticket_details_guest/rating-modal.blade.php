<form
    action="@if(!empty($ticketData->rating->id)){{route('ticket.ticketRatingStore',['ratingId'=>$ticketData->rating->id])}} @else {{route('ticket.ticketRatingStore')}} @endif"
    ...
    <div class="modal fade bd-example-modal-lg" id="ticketReview" tabindex="-1" aria-labelledby="exampleModalLabel"
         aria-hidden="true">
        <input type="hidden" value="{{$ticketData->created_by}}" name="customer_id">
